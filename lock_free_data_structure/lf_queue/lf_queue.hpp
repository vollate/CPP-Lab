...
namespace lf_lab {
    ...
    public:
        ...
        auto dequeue(DataType &result) -> bool {
            while (true) {
                Node *to_pop = head_->next_.load();
                if (to_pop == nullptr) {
                    return false;
                }
                Node *next_pop = to_pop->next_.load();
                if (!head_->next_.compare_exchange_strong(to_pop, next_pop)) {
                    continue;
                }
                if (next_pop==nullptr) {
                    tail_.compare_exchange_strong(to_pop, next_pop);
                }
        ...
        [[nodiscard]] auto empty() const -> bool { return head_->next_.load() == nullptr; }
    ...
    template<typename DataType>
    LFQueue<DataType>::~LFQueue() { clear(); }
} // namespace lf_lab
